...
export async function getGalleriesService() {
  try {
    const galleriesDB = await Gallery.find().populate({
      path: 'creatorId',
      select: 'username',
    });
...
export async function postGalleryService(form, userId) {
  try {
    const newDir = path.resolve(__dirname, 'static/images/gallery', form.nameDir);
    ...
    await sharp(myBuffer)
      ...
      .toFormat('jpeg')
      .toFile(
        path.resolve(__dirname, 'static/images/gallery', form.nameDir, `gallery-cover.jpg`)
      );
...
export async function postAlbumService(form, userId) {
  try {
    ...
    const newDir = path.resolve(__dirname, 'static', galleryDB.urlGallery, form.nameDir);
    if (fs.existsSync(newDir)) throw `Такая "${form.nameDir}" папка уже существует`;
...
export async function deleteGalleryService(galleryId) {
  try {
    ...
    fs.rmSync(path.resolve(__dirname, 'static', galleryDB.urlGallery), { recursive: true });
    const albumsDB = await Album.find({ galleryId });
...
export async function deleteAlbumService(albumId) {
  try {
    ...
    fs.rmSync(path.resolve(__dirname, 'static', albumDB.urlAlbum), { recursive: true });
...
export async function deletePhotoService(photoId) {
  try {
    ...
    fs.unlinkSync(path.resolve(__dirname, 'static', photoDB.urlPhotoSmall));
    fs.unlinkSync(path.resolve(__dirname, 'static', photoDB.urlPhotoMedium));
    fs.unlinkSync(path.resolve(__dirname, 'static', photoDB.urlPhotoNormal));
